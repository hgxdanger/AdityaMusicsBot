"""
    ...
    aditya-userbot, Telegram Voice Chat Userbot
...
USERBOT_HELP = f"""{emoji.LABEL}  **Ʌɗɩtƴɑ Ƈøɱɱøɳ Ƈøɱɱɑɳɗs**:
__available to group members of current voice chat__
__starts with . (fullstop mark)__
...
USERBOT_REPO = f"""{emoji.ROBOT} **Ʌɗɩtƴɑ Ƭɘɭɘʛrɑɱ Ɱʉsɩƈ Ƥɭɑƴɘr**

- Øωɳɘr: [Ʌɗɩtƴɑ Ɦɑɭɗɘr](https://t.me/AdityaHalder)
- Ʀɘƥøs: Ƈøɳtɑƈt Øωɳɘr"""
...
async def play_track(client, m: Message):
    ...
    if m.audio:
        if m.audio.duration > (DURATION_AUTOPLAY_MIN * 60):
            reply = await m.reply_text(
                f"{emoji.ROBOT} Ʌʉɗɩø Wɦɩƈɦ Ɗʉrɑtɩøɳ Løɳʛɘr Ƭɦɑɳ "
                f"{str(DURATION_AUTOPLAY_MIN)} Ɱɩɳ Wøɳ't Ɓɘ Ʌʉtøɱɑtɩƈɑɭɭƴ "
                "Ʌɗɗɘɗ Ƭø Ƥɭɑƴɭɩst"
            )
        ...
        if m_audio.audio.duration > (DURATION_PLAY_HOUR * 60 * 60):
            reply = await m.reply_text(
                f"{emoji.ROBOT} Ʌʉɗɩø Wɦɩƈɦ Ɗʉrɑtɩøɳ Løɳʛɘr Ƭɦɑɳ "
                f"{str(DURATION_PLAY_HOUR)} Ɦøʉrs Wøɳ't Ɓɘ Ʌɗɗɘɗ Ƭø Ƥɭɑƴɭɩst"
            )
            await _delay_delete_messages((reply,), DELETE_DELAY)
            return
    else:
    ...
    if playlist and playlist[-1].audio.file_unique_id \
        ...
        reply = await m.reply_text(f"{emoji.ROBOT} Ʌɭrɘɑɗƴ Ʌɗɗɘɗ")
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} Ɗøωɳɭøɑɗɩɳʛ Ʌɳɗ Ƭrɑɳsƈøɗɩɳʛ..."
        )
        ...
        print(f"- Stɑrt Ƥɭɑƴɩɳʛ: {playlist[0].audio.title}")
    await mp.send_playlist()
...
async def skip_track(_, m: Message):
    ...
    if len(m.command) == 1:
        ...
        try:
            ...
            reply = await m.reply_text(f"{emoji.NO_ENTRY} Iɳⱱɑɭɩɗ Iɳƥʉt",
...
async def join_group_call(client, m: Message):
    ...
    if group_call.is_connected:
        await m.reply_text(f"{emoji.ROBOT} Ʌɭrɘɑɗƴ Jøɩɳɘᶑ Ʌ Vøɩƈɘ Ƈɦɑt")
        return
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        reply = await m.reply_text(
            f"{emoji.MUSICAL_NOTES} **Ƈʉrrɘɳtɭƴ Iɳ Ƭɦɘ Vøɩƈɘ Ƈɦɑt**:\n"
            f"- **{chat.title}**"
        )
    else:
        reply = await m.reply_text(emoji.NO_ENTRY
                                   + "Ɗɩɗɳ't Jøɩɳ Ʌɳ Vøɩƈɘƴ Ƈɦɑt Yɘt")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def stop_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.STOP_BUTTON} Støƥƥɘɗ Ƥɭɑƴɩɳʛ")
...
async def pause_playing(_, m: Message):
    ...
    await mp.update_start_time(reset=True)
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Ƥɑʉsɘɗ",
                               quote=False)
    mp.msg['pause'] = reply
    await m.delete()
...
async def resume_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Ʀɘsʉɱɘɗ",
                               quote=False)
...
async def clean_raw_pcm(client, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.WASTEBASKET} Ƈɭɘɑɳɘɗ {count} Fɩɭɘs")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def mute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.MUTED_SPEAKER} Ɱʉtɘɗ")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} Ʉɳɱʉtɘɗ")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
