...
class MusicPlayer(object):
    ...
    async def send_playlist(self):
        ...
        if not playlist:
            pl = f"{emoji.NO_ENTRY} Eɱƥtƴ Ƥɭɑƴɭɩst"
        else:
            if len(playlist) == 1:
                pl = f"{emoji.REPEAT_SINGLE_BUTTON} **Ƥɭɑƴɭɩst**:\n"
            else:
                pl = f"{emoji.PLAY_BUTTON} **Ƥɭɑƴɭɩst**:\n"
            pl += "\n".join([
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await mp.send_text(f"{emoji.CHECK_MARK_BUTTON} Jøɩɳɘɗ Ƭɦɘ Vøɩƈɘ Ƈɦɑt")
    else:
        await mp.send_text(f"{emoji.CROSS_MARK_BUTTON} Lɘfʈ Ƭɦɘ Vøɩƈɘ Ƈɦɑt")
        mp.chat_id = None
